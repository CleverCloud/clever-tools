...
```bash
clever database backups [FLAGS] <DATABASE-ID>
```
...
| Name | Description |
|------|-------------|
| `database-id` | Any database ID (format: addon\_UUID, postgresql\_UUID, mysql\_UUID, ...) **(required)** |
...
```bash
clever database backups download [FLAGS] <BACKUP-ID> <DATABASE-ID>
```
...
| Name | Description |
|------|-------------|
| `backup-id` | A Database backup ID (format: UUID) **(required)** |
| `database-id` | Any database ID (format: addon\_UUID, postgresql\_UUID, mysql\_UUID, ...) **(required)** |
