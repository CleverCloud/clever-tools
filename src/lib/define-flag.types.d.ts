...
/**
 * Flag definition shape with Zod schema for type inference.
 *
 ...
 * });
 */
export interface FlagDefinition<S extends z.ZodType = z.ZodType> {
  /**
   * The CLI flag name used by the parser (e.g., 'region' for --region).
   *
   * This is the name shown to users in help text and used on the command line.
   * The object key in the command's flags record is only for internal code access.
   *
  ...
  /**
   * Zod schema defining the flag's type, default value, and validation.
   *
   * The schema determines:
   * - Type (string, boolean, number, enum, etc.)
   * - Whether the flag is required (no default) or optional (has default)
   ...
   * @example z.boolean().default(false) // boolean flag
   ...
   *
   * Deprecated flags still work but show warnings when used.
   *
   * @example true
 ...
 */
export type InferFlagType<S extends z.ZodType> = z.infer<S>;
 ...
 * @param definition - Flag definition object
 ...
 *   },
 *   handler: async (flags) => {
 *     // flags.region is string, flags.verbose is boolean
 *   },
 * });
 */
export function defineFlag<S extends z.ZodType>(definition: FlagDefinition<S>): FlagDefinition<S>;
