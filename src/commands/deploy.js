...
async function deploy (params) {
  const { alias, branch: branchName, tag: tagName, quiet, force, follow, 'same-commit-policy': sameCommitPolicy, watch } = params.options;
  ...
  if (commitIdToPush === remoteHeadCommitId) {
    switch (sameCommitPolicy) {
      case 'ignore':
        ...
        return;
      case 'restart':
        return restartOnSameCommit(ownerId, appId, commitIdToPush, quiet, follow, false, watch);
      case 'rebuild':
        return restartOnSameCommit(ownerId, appId, commitIdToPush, quiet, follow, true, watch);
      case 'error':
      default: {
  ...
  return Log.watchDeploymentAndDisplayLogs({ ownerId, appId, commitId: commitIdToPush, knownDeployments, quiet, follow, watch });
}

async function restartOnSameCommit (ownerId, appId, commitIdToPush, quiet, follow, withoutCache, watch) {
  const restart = await Application.redeploy(ownerId, appId, commitIdToPush, withoutCache);
  return Log.watchDeploymentAndDisplayLogs({ ownerId, appId, deploymentId: restart.deploymentId, quiet, follow, watch });
}
