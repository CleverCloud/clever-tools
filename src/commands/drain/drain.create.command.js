...
export const drainCreateCommand = defineCommand({
  ...
  options: {
    ...
    indexPrefix: defineOption({
      name: 'index-prefix',
      schema: z.string().optional(),
      description: 'Optional index prefix, `logstash` value is used if not set',
      aliases: ['i'],
    ...
    rfc5424StructuredDataParameters: defineOption({
      name: 'sd-params',
      schema: z.string().optional(),
      description: 'RFC5424 structured data parameters, e.g.: `X-OVH-TOKEN=\\\"REDACTED\\\"`',
      aliases: ['s'],
      placeholder: 'sd-params',
      groups: ['ovh-tcp', 'syslog-tcp', 'syslog-udp'],
    }),
    alias: aliasOption,
