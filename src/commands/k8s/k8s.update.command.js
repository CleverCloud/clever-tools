...
export const k8sUpdateCommand = defineCommand({
  ...
  options: {
    ...
    disablePersistentStorage: defineOption({
      name: 'disable-persistent-storage',
      schema: z.boolean().default(false),
      description: 'Disable persistent storage (Ceph CSI)',
    }),
  ...
  async handler(options, clusterIdOrName) {
    ...
    if (options.persistentStorage && options.disablePersistentStorage) {
      throw new Error('--persistent-storage and --disable-persistent-storage are mutually exclusive');
    }
    ...
    if (options.disablePersistentStorage) features.csi = false;
    if (Object.keys(features).length > 0) updates.features = features;

    if (Object.keys(updates).length === 0) {
      throw new Error(
        'No update specified. Provide at least one of --name, --description, --tag, --autoscaling, --disable-autoscaling, --persistent-storage, --disable-persistent-storage',
      );
    }
