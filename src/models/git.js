 ...
 * Implementations: GitIsomorphic (JS-based) and GitNative (native git command)
 */
export class Git {
  /** @type {Git | null} */
  ...
  static async get() {
    if (Git.#instance == null) {
      const useNativeGit = await isFeatureEnabled('native-git');
      if (useNativeGit) {
        const { GitNative } = await import('./git-native.js');
        Git.#instance = new GitNative();
      } else {
