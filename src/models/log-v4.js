...
export async function watchDeploymentAndDisplayLogs (options) {
  ...
  if (deploymentEnded.state === 'OK') {
    ...
    Logger.println('');
    Logger.println(colors.bold.green('✓ Access your application at'), colors.underline.bold(`https://${favouriteDomain.fqdn}`));
    Logger.println(colors.bold.blue('→ Manage your application at'), colors.underline.bold(`https://console.clever-cloud.com/goto/${appId}`));
  }
