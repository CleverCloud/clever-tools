 ...
 * to:   function cmd(flags, y) { const { x } = flags; }
 */
...
function buildNewParams(usage, cmdInfo) {
  ...
  // Add flags parameter (or _flags if unused)
  if (hasArgs || needsFlags) {
    const flagsName = needsFlags ? 'flags' : '_flags';
    newParams.push(t.identifier(flagsName));
  }
...
function transformFunctionBody(body, usage) {
  ...
  // Replace params.options with flags, and handle params.args patterns
  traverse.default(
    t.file(t.program([t.isStatement(body) ? body : t.expressionStatement(body)])),
    {
      CallExpression(path) {
      ...
      MemberExpression(path) {
        if (
          ...
          path.replaceWith(t.identifier('flags'));
        }
