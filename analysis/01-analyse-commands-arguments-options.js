...
function analyzeCommands() {
      ...
      if (t.isIdentifier(node.id, { name: 'commands' }) && t.isArrayExpression(node.init)) {
        ...
        node.init.elements.forEach((element) => {
          if (t.isIdentifier(element)) {
            ...
            // Handle cliparseCommands.helpCommand
            if (
              t.isIdentifier(element.object, { name: 'cliparseCommands' }) &&
              t.isIdentifier(element.property, { name: 'helpCommand' })
            ) {
              usedCommands.add('helpCommand');
            }
          }
        });
      }
  ...
  commandDefinitions.set('helpCommand', {
    name: 'help',
    ...
    for (const subcommandName of subcommandNames) {
      if (commandDefinitions.has(subcommandName)) {
        ...
        resolved.push({
          ...
          subcommands: resolveSubcommands(subcommand.subcommands || []),
        });
      }
    }
    return resolved;
  ...
  for (const [variableName, commandData] of commandDefinitions) {
    if (usedCommands.has(variableName)) {
      ...
      finalCommands.push({
        ...commandData,
        experimental: isExperimental,
        subcommands: resolveSubcommands(commandData.subcommands || []),
      });
    }
  }
...
try {
  ...
  // Write to JSON file
  const outputPath = path.join(process.cwd(), 'analysis', 'data', 'analyse-1-commands-arguments-options.json');
