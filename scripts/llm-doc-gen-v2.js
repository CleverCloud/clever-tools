...
const GLOBAL_FLAG_NAMES = ['help', 'version', 'color', 'update-notifier', 'verbose'];
...
/**
 * Format a flag for display in the options list.
 * Format: [--name, -a] PLACEHOLDER    Description (default: value)
 */
function formatFlagLine(name, flag) {
  const parts = [];

  // Build the flag names part: [--name, -a, --alias]
  const names = [];
  names.push(`--${name}`);
  if (flag.aliases) {
  ...
  // Add placeholder for non-boolean flags
  const isBoolean = isBooleanFlag(flag);
  if (!isBoolean) {
    const placeholder = flag.placeholder || name;
  ...
  // Combine: pad the flag+placeholder part to align descriptions
  const flagPart = parts.join(' ');
  return `${flagPart.padEnd(35)}${description}`;
}
...
function formatCommandDoc(cmd, level = 1, isRoot = false) {
  ...
  // Options
  const flags = cmd.command.flags || {};
  const flagEntries = Object.entries(flags);

  // Filter out global flags for non-root commands
  const filteredFlags = isRoot
    ? flagEntries
    : flagEntries.filter(([name]) => !GLOBAL_FLAG_NAMES.includes(name));

  // Sort flags alphabetically by name
  filteredFlags.sort(([a], [b]) => a.localeCompare(b));

  if (filteredFlags.length > 0) {
    result.push('**Options:**');
    result.push('```');
    for (const [name, flag] of filteredFlags) {
      result.push(formatFlagLine(name, flag));
    }
    result.push('```');
    result.push('');
  }
