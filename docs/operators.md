...
Then, you can use the `clever operators` command to manage Keycloak, Matomo, Metabase and Otoroshi instances on Clever Cloud. For example to list them deployed services:
...
```
clever otoroshi open logs myKeycloak
clever otoroshi open webui keycloak_id
```
...
clever otoroshi ng disable otoroshi_id
```
