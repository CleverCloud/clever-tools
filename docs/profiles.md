...
Clever Tools can store multiple login profiles in its configuration file. Each profile contains your authentication data and an optional set of overrides for platform endpoints (API host, Console URL, etc.). The active profile is the first one in the list and is used for all commands.
