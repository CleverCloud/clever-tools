...
async function run () {
  ...
  // ARGUMENTS
  const args = {
    kvRawCommand: cliparse.argument('command', { description: 'The raw command to send to the MateriaDB KV or Redis® add-on' }),
    kvIdOrName: cliparse.argument('kv-id', {
      description: 'Add-on/Real ID (or name, if unambiguous) of a MateriaDB KV or Redis® add-on',
    }),
  ...
  const kvRawCommand = cliparse.command('kv', {
    description: 'Send a raw command to a MateriaDB KV or Redis® add-on',
  ...
  if (featuresFromConf.kv) commands.push(colorizeExperimentalCommand(kvRawCommand, 'kv'));
